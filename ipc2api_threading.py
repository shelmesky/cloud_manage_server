        ...
        while 1:
            ...
            for ins in item:
                ...
                if not q.all():
                    ...
                    q.update({Instances.ip_address:simplejson.dumps(ins['ip_address']), \
                              Instances.name:ins['name'], \
                              Instances.uuid:ins['id'],Instances.state:ins['power_state'], \
                              Instances.physical_host:ins['physical_host']
                    })
                    session.commit()
    ...
    def run(self):
        while 1:
            ...
            if s_item[0] == "notification":
                ...
                # return code 2 and 1 will be use in the future
                if return_code == '2':
                    break
                elif return_code == '1':
                    break
...
class get_nova_instances_config(threading.Thread):
    """
    Get configuration for instances from remote server.
    """
    def __init__(self):
        super(get_nova_instances_config,self).__init__()
        self.daemon = False
...
class check_nova_instances(threading.Thread):
    def __init__(self,queue):
    ...
    def run(self):
        while 1:
            try:
                ...
                logger.error("Error happend while check instances.")
                logger.exception(e)
            time.sleep(60)
